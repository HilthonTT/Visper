...
func WithHTTPClient(client HttpClient) RequestOption {
	return requestconfig.RequestOptionFunc(func(rc *requestconfig.RequestConfig) error {
		if client == nil {
			return fmt.Errorf("requestoption: WithHTTPClient received nil client")
		}

		if c, ok := client.(*http.Client); ok {
			// Prefer the native client if possible.
			rc.HTTPClient = c
			rc.CustomHTTPDoer = nil
		} else {
			rc.CustomHTTPDoer = client
		}
...
func WithMaxRetries(retries int) RequestOption {
	if retries < 0 {
		panic("requestoption: WithMaxRetries received negative retries")
	}
...
func WithQueryAdd(key, value string) RequestOption {
	return requestconfig.RequestOptionFunc(func(rc *requestconfig.RequestConfig) error {
		...
		query.Del(key)
		rc.Request.URL.RawQuery = query.Encode()
		return nil
	})
}
...
func WithJSONSet(key string, value any) RequestOption {
	return requestconfig.RequestOptionFunc(func(rc *requestconfig.RequestConfig) (err error) {
		var b []byte
		...
		} else if buffer, ok := rc.Body.(*bytes.Buffer); ok {
			b = buffer.Bytes()
...
func WithJSONDel(key string) RequestOption {
	return requestconfig.RequestOptionFunc(func(rc *requestconfig.RequestConfig) (err error) {
		if buffer, ok := rc.Body.(*bytes.Buffer); ok {
			b := buffer.Bytes()
...
func WithRequestBody(contentType string, body any) RequestOption {
	return requestconfig.RequestOptionFunc(func(rc *requestconfig.RequestConfig) error {
		if reader, ok := body.(io.Reader); ok {
			rc.Body = reader
			return rc.Apply(WithHeader("Content-Type", contentType))
		}
...
func WithEnvironmentProduction() RequestOption {
	return requestconfig.WithDefaultBaseURL("https://visper.com/")
}
...
func WithEnvironmentDev() RequestOption {
	return requestconfig.WithDefaultBaseURL("https://localhost:8080/")
}
...
func WithAppID(value string) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.AppID = value
		return r.Apply(WithHeader("x-visper-app-id", value))
	})
}
