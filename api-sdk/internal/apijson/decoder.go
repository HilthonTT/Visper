...
func (d *decoderBuilder) newTimeTypeDecoder(t reflect.Type) decoderFunc {
	format := d.dateFormat
	return func(node gjson.Result, value reflect.Value, state *decoderState) error {
		parsed, err := time.Parse(format, node.Str)
		if err == nil {
			value.Set(reflect.ValueOf(parsed).Convert(t))
			return nil
		}
		...
		for _, layout := range layouts {
			parsed, err := time.Parse(layout, node.Str)
			if err == nil {
				value.Set(reflect.ValueOf(parsed).Convert(t))
				return nil
			}
		...
		return fmt.Errorf("unable to leniently parse date-time string: %s", node.Str)
	}
}
...
func (d *decoderBuilder) newUnionDecoder(t reflect.Type) decoderFunc {
	...
	return func(node gjson.Result, value reflect.Value, state *decoderState) error {
		// If there's a discriminator match, circumvent the exactness logic entirely
		for idx, variant := range unionEntry.variants {
			decoder := decoders[idx]
			if variant.TypeFilter != node.Type {
				continue
			}

			if len(unionEntry.discriminatorKey) != 0 {
				discriminatorValue := node.Get(EscapeSJSONKey(unionEntry.discriminatorKey)).Value()
				if discriminatorValue == variant.DiscriminatorValue {
					inner := reflect.New(variant.Type).Elem()
					err := decoder(node, inner, state)
					value.Set(inner)
					return err
				}
			}
...
func guardStrict(state *decoderState, condition bool) bool {
	if !condition {
		return false
	}
