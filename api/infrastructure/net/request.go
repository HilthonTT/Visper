...
func (d *downloader) download() (io.ReadCloser, error) {
	...
	log.Printf("cfgConcurrency:%d", d.cfg.Concurrency)
...
func (d *downloader) interrupt() error {
	if d.written != d.params.Range.Length {
		log.Println("Downloader interrupt before finish")
		if d.getErr() == nil {
	...
	defer func() {
		...
		log.Printf("maxConcurrency:%d", d.cfg.Concurrency+d.concurrency)
	}()
	return d.err
}
...
func (d *downloader) downloadChunk(ch *chunk) error {
	log.Printf("start chunk_%d, %+v", ch.id, ch)
	...
	for retry := 0; retry <= d.cfg.PartBodyMaxRetries; retry++ {
		...
		if err == nil {
			d.incrWritten(n)
			log.Printf("chunk_%d downloaded", ch.id)
			break
		}
		if d.getErr() != nil {
			return nil
		...
		if e, ok := err.(*errNeedRetry); ok {
			...
			log.Printf("err chunk_%d, object part download error %s, retrying attempt %d. %v",
...
func (d *downloader) tryDownloadChunk(params *HttpRequestParams, ch *chunk) (int64, error) {
	...
	if err != nil {
		...
		// 来到这 说明第1个分片下载 连接成功了
		// 后续分片下载出错都当超载处理
		log.Printf("err chunk_%d, try downloading:%v", ch.id, err)
...
func (mr MultiReadCloser) Read(p []byte) (n int, err error) {
	...
	if err == io.EOF {
		log.Printf("read_%d finished current buffer", mr.cfg.rPos)
