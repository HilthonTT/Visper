...
func ServeHTTP(w http.ResponseWriter, r *http.Request, name string, modTime time.Time, size int64, RangeReadCloser model.RangeReadCloserIF) error {
	...
	if r.Method != "HEAD" {
		...
		if err != nil {
			log.Printf("ServeHttp error. err: %s ", err)
			if written != sendSize {
				log.Printf("Maybe size incorrect or reader not giving correct/full data, or connection closed before finish. written bytes: %d ,sendSize:%d, ", written, sendSize)
			}
...
func ProcessHeader(origin, override http.Header) http.Header {
	...
	for h, val := range origin {
		if utils.SliceContains(config.SlicesMap[config.ProxyIgnoreHeaders], strings.ToLower(h)) {
			continue
		}
		result[h] = val
	}
...
func NewHttpClient() *http.Client {
	return &http.Client{
		...
		Transport: &http.Transport{
			...
			TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, // TODO: Add proper config
		},
	}
}
