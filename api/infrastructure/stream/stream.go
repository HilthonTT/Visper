...
func (f *FileStream) Close() error {
	...
	if f.tmpFile != nil {
		...
		if err2 != nil {
			err2 = fmt.Errorf("failed to remove tmpFile [%s]: %v", err2, f.tmpFile.Name())
		} else {
			f.tmpFile = nil
		}
	}
...
func (c *headCache) read(p []byte) (n int, err error) {
	pL := len(p)
	log.Printf("headCache read_%d", pL)
	if c.cur < int64(pL) {
		bufL := int64(pL) - c.cur
		buf := make([]byte, bufL)
...
func (r *RangeReadReadAtSeeker) getReaderAtOffset(off int64) (*readerCur, error) {
	...
	if rc != nil && off-rc.cur <= utils.MB {
		...
		if err == nil {
			log.Printf("getReaderAtOffset old_%d", off)
			return rc, nil
		}
		rc.cur = -1
	}
	log.Printf("getReaderAtOffset new_%d", off)
