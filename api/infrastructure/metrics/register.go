...
func NewMetricsManager(meter metric.Meter, logger *logger.Logger) Manager {
...
func (m *metricsManager) NewGauge(name, desc string) {
	...
	if err != nil {
		m.logger.Error("new-gauge", zap.Error(err))
		return
	}

	err = m.store.setGauge(name, gauge)
	if err != nil {
		m.logger.Error("set-gauge", zap.Error(err))
	}
}
...
func (m *metricsManager) NewCounter(name, desc string) {
	...
	if err != nil {
		m.logger.Error("new-counter", zap.Error(err))
		return
	}

	err = m.store.setCounter(name, counter)
	if err != nil {
		m.logger.Error("set-counter", zap.Error(err))
		return
	}
}
...
func (m *metricsManager) NewUpDownCounter(name, desc string) {
	...
	if err != nil {
		m.logger.Error("new-up-down-counter", zap.Error(err))
		return
	}

	err = m.store.setUpDownCounter(name, upDownCounter)
	if err != nil {
		m.logger.Error("set-up-down-counter", zap.Error(err))
	}
}
...
func (m *metricsManager) NewHistogram(name, desc string, buckets ...float64) {
	...
	if err != nil {
		m.logger.Error("new-histogram", zap.Error(err))
		return
	}

	err = m.store.setHistogram(name, histogram)
	if err != nil {
		m.logger.Error("set-histogram", zap.Error(err))
	}
}
...
func (m *metricsManager) IncrementCounter(ctx context.Context, name string, labels ...string) {
	...
	if err != nil {
		m.logger.Error("increment-counter", zap.Error(err))
		return
	}
...
func (m *metricsManager) DeltaUpDownCounter(ctx context.Context, name string, value float64, labels ...string) {
	...
	if err != nil {
		m.logger.Error("delta-up-down-counter", zap.Error(err))
		return
	}

	upDownCounter.Add(ctx, 1, metric.WithAttributes(m.getAttributes(name, labels...)...))
}
...
func (m *metricsManager) RecordHistogram(ctx context.Context, name string, value float64, labels ...string) {
	...
	if err != nil {
		m.logger.Error("record-histogram", zap.Error(err))
		return
	}
...
func (m *metricsManager) getAttributes(name string, labels ...string) []attribute.KeyValue {
	...
	if labelsCount%2 != 0 {
		m.logger.Warn(fmt.Sprintf("metrics %v label has invalid key-value pairs", name))
	}

	cardinalityLimit := 20
	if labelsCount > cardinalityLimit {
		m.logger.Warn(fmt.Sprintf("metrics %v has high cardinality: %v", name, labelsCount))
	}

	var attributes []attribute.KeyValue
	if labels != nil {
		for i := 0; i < labelsCount-1; i += 2 {
