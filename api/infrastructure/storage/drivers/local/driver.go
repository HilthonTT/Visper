...
import (
	...
	"github.com/hilthontt/visper/api/infrastructure/sign"
...
func (d *Local) Config() drivers.Config {
	return driverConfig
}
...
func (d *Local) GetAddition() drivers.Additional {
	return &d.Addition
}

func (d *Local) List(ctx context.Context, dir model.Obj, args model.ListArgs, cfg *config.Config) ([]model.Obj, error) {
	...
	for _, f := range rawFiles {
		...
		file := d.FileInfoToObj(ctx, f, args.ReqPath, fullPath, cfg)
...
func (d *Local) FileInfoToObj(ctx context.Context, f fs.FileInfo, reqPath string, fullPath string, cfg *config.Config) model.Obj {
	thumb := ""

	if d.Thumbnail {
		typeName := utils.GetFileType(f.Name())
		if typeName == config.IMAGE || typeName == config.VIDEO {
			thumb = utils.GetApiUrl(utils.GetHttpReq(ctx), cfg) + path.Join("/d", reqPath, f.Name())
			thumb = utils.EncodePath(thumb, true)
			thumb += "?type=thumb&sign=" + sign.Sign(path.Join(reqPath, f.Name()))
		}
	}
...
func (d *Local) GetMeta(ctx context.Context, path string, cfg *config.Config) (model.Obj, error) {
	f, err := os.Stat(path)
	if err != nil {
		return nil, err
	}
	file := d.FileInfoToObj(ctx, f, path, path, cfg)
	//h := "123123"
...
func (d *Local) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up drivers.UpdateProgress) error {
...
var _ drivers.Driver = (*Local)(nil)
