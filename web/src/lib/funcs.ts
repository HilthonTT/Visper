 ...
 * @returns {string} tabs - The tab string
 */
export function generateTabs(num: number = 0): string {
  let tabs = "\xA0\xA0";
  for (let i = 0; i < num; i++) {
    tabs += "\xA0";
  }
...
export function isArgInvalid(
  ...
  return arg[0] !== action || !_.includes(options, arg[1]) || arg.length > 2;
}
 ...
 */
export function getCurrentCmdArray(history: string[]): string[] {
  return _.split(history[0].trim(), " ");
}
